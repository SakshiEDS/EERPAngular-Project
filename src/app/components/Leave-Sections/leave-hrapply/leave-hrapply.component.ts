...
@Component({
  selector: 'app-leave-hrapply',
  templateUrl: './leave-hrapply.component.html',
  styleUrl: './leave-hrapply.component.scss',
  standalone: true,
...
export class LeaveHrapplyComponent implements OnInit {
  voucherDate: Date | null = null;
  canAddLeaveHrApp: boolean = false;
  ...
  ngOnInit() {
    ...
    this.canAddLeaveHrApp = this.permissionService.hasPermission('LeaveHrapplyComponent', 'LeaveHrapplyComponentAdd');
  }
  ...
  isSubmitting = false;
  submitHRApplication(): void {
    if (!this.canAddLeaveHrApp) {
    ...
    if (!leaveCode) {
      this.messageService.add({ severity: 'error', summary: 'Leave Code Error', detail: 'Leave code not found for the selected leave type.' });
      return;
    }
    ...
    const payload = {
      ...
      managerCode: this.managercode, // Always null
      leaveCode: leaveCode, // Use the dynamically fetched leave code
      ...
      leaveRemark: this.remark, // Always null
      leaveStatus: null, // Always null
      leaveDays: this.days,
      leaveActionDate: null, // Always null
    };
    ...
    this.LeaveMasterService.submitLeaveApplication(payload).subscribe({
      next: (response) => {
        ...
        this.holidayService.triggerRefresh(); // Trigger refresh
